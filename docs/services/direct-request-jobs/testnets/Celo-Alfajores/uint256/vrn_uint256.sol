...
contract LinkWellVRNUint256ConsumerContractExample is ChainlinkClient, ConfirmedOwner {
    ...
    event RequestFulfilled(bytes32 requestId, uint256 randomNumber, bytes32 hash);
    function fulfill(bytes32 requestId, uint256 _randomNumber, bytes32 hash) public recordChainlinkFulfillment(requestId) {
        ...
        require(((uint256(
                    keccak256(                
                        abi.encodePacked(
                            ...
                            requestId, 
                            index))) %
                (req.maxVal - req.minVal + 1)) + req.minVal) == req.randomNumber,
