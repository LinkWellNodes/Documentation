...
contract LinkWellVRNInt256ConsumerContractExample is ChainlinkClient, ConfirmedOwner {
    ...
    /**
     * Send a request to the Chainlink oracle to retrieve a deterministic 'random' integer within the given range (inclusive).
    ...
    int256 public latestRandomNumber;
    bytes32 public latestFulfilledRequestId;

    // Receive the result from the Chainlink oracle
    event RequestFulfilled(bytes32 requestId, int256 randomNumber, bytes32 hash);
    function fulfill(bytes32 requestId, int256 _randomNumber, bytes32 hash) public recordChainlinkFulfillment(requestId) {
    ...
    function storeResultForVerification(bytes32 requestId, int256 _randomNumber, bytes32 hash) internal {
    ...
    function getRandomNumber(bytes32 requestId) internal view returns (int256) {
    ...
    function verifyResult(bytes32 requestId, bytes32 seed) public view returns (bool) {
        ...
        uint256 index = 0;  // always 0 for single-number requests
        ...
        require((int256(uint256(
                    keccak256(                
                        abi.encodePacked(
                            ...
                            requestId, 
                            index))) %
                uint256(req.maxVal - req.minVal + 1)) + req.minVal) == req.randomNumber,
